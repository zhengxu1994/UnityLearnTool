Shader "MoreMountains/SpriteHDR"
{
		...
		Pass
	{
		...
		int InsideGlow(fixed4 sampledColor, float2 texCoord, int isOutlineEnabled, int outlineSize, float alphaThreshold)
		{
		...
		int OutsideGlow(fixed4 sampledColor, float2 texCoord, int isOutlineEnabled, int outlineSize, float alphaThreshold)
		{
		...
		fixed4 ComputeFragment(VertexOutput vertexOutput) : SV_Target
		{
			...
			#else
				int shouldDrawOutline = InsideGlow(color, vertexOutput.TexCoord, isOutlineEnabled, outlineSize, alphaThreshold);
			#endif
