...
namespace CSHotFix.Reflection
{
    public class CSHotFixType : Type
    {
        ...
        void InitializeProperties()
        {
            if (type.TypeDefinition == null)
            {
                properties = new CSHotFixPropertyInfo[0];
                return;
            }
        ...
        public override Type MakeGenericType(params Type[] typeArguments)
        {
            if (ILType.TypeReference.HasGenericParameters)
            {
                ...
                for (int i = 0; i < ga.Length; i++)
                {
                    ...
                    if (typeArguments[i] is CSHotFixType)
                        ga[i] = new KeyValuePair<string, IType>(key, ((CSHotFixType)typeArguments[i]).ILType);
                    else
        ...
        public override bool IsAssignableFrom(Type c)
        {
            ...
            {
                type = ((CSHotFixWrapperType)c).CLRType;
            }
            else if (c is CSHotFixType)
            {
                type = ((CSHotFixType)c).ILType;
            }
            else
