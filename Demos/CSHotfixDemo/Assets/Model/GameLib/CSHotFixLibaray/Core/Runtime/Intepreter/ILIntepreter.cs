...
using CSHotFix.Runtime.Intepreter.OpCodes;
...
#if DEBUG && (UNITY_EDITOR || UNITY_ANDROID || UNITY_IPHONE)
                                                    string InvokeName = cm.ToString();
                                                   // if (InvokeName != "Void Invoke(WfPacket)")
                                                    {
                                                        UnityEngine.Debug.LogWarning("Not Binding, Invoke:" + InvokeName + 
                                                            "Detail:" + (method.DeclearingType != null ? method.DeclearingType.ToString() : " ") +  " " + method.Name);
                                                    }
#endif
#if UNITY_EDITOR
                                                    if (System.Threading.Thread.CurrentThread.ManagedThreadId == AppDomain.UnityMainThreadID)
                            ...
                            case OpCodeEnum.Ldelem_Any:
                                {
                                    ...
                                    {
                                        ILTypeInstance ins = (ILTypeInstance)obj;
                                        if (ins != null && ins.Type != null && ins.Type.IsValueType && !ins.Boxed)
                                        {
...
#region Stack operation
                    ...
                    catch (Exception ex)
                    {
                        if (method.ExceptionHandler != null)
                        {
                            ...
                            if (eh != null)
                            {
                                if (ex is CSHotFixException)
                                {
                                    CSHotFixException ire = (CSHotFixException)ex;
                        ...
                        var newEx = new CSHotFixException(ex.Message, this, method, ex);
                        throw newEx;
                    }
                }
            }
        ...
        ExceptionHandler GetCorrespondingExceptionHandler(ILMethod method, object obj, int addr, ExceptionHandlerType type, bool explicitMatch)
        {
            ...
            Exception ex = obj is CSHotFixException ? ((CSHotFixException)obj).InnerException : obj as Exception;
